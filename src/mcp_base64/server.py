...
def _decode_base64_to_file_impl(
    ...
    success_msg = f"Successfully decoded base64 content to file: {resolved_path}"
    return success_msg
...
def encode_file_to_base64(
    file_path: str = Field(
        ...
        examples=["/home/user/document.pdf", "/tmp/image.png"]
    )
) -> str:
    """
    Encode a file to a base64 string.

    This tool reads a file in binary mode and returns its contents encoded as a base64 string.
...
def decode_base64_to_file(
    base64_content: str = Field(
        ...
        examples=["SGVsbG8gV29ybGQ=", "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNk+M9QDwADhgGAWjR9awAAAABJRU5ErkJggg=="]
    ),
    file_path: str = Field(
        description="Absolute path where to save the decoded file",
        examples=["/home/user/decoded.pdf", "/tmp/output.png"]
    )
) -> str:
    """
    Decode a base64 string and write it to a file.

    This tool takes base64 encoded content and writes the decoded binary data to a file.
    Only absolute paths are allowed for security reasons.
    ...
    Raises:
        ToolError: If base64 is invalid, file cannot be written, path is invalid, or other error occurs
    """
